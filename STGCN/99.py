...
from ours.load import haversine, load_traj
...
for i in range(1000):
    ...
    DB = DBSCAN(eps=0.01, min_samples=5).fit(pos[:, 1:]).labels_
    print(DB)
    print('??????????')
    pos0 = []
    num_group = DB.max() + 1
    plt.figure()
    plt.scatter(pos[:, 2], pos[:, 1], color='black', s=1)  # 2， 1： longitude, latitude; 经度，纬度
    for num in range(num_group):
        ...
        for i, item in enumerate(pos):
            if DB[i] == num:
                pos0.append(item)
        pos0 = np.array(pos0)
        plt.scatter(pos0[:, 2], pos0[:, 1], color=color[num], s=1)
        pos0 = []
    plt.show()
    plt.close()
